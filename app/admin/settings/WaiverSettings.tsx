...
import VersionField from "../waiver/VersionField";
...
async function saveWaiver(formData: FormData) {
  ...
  if (featureWaiverRequired && !waiverText) {
    redirect("/admin/settings?waiverError=text_required#liability-waiver");
  }
  if (manualVersion && !typedVersion) {
    redirect("/admin/settings?waiverError=version_required#liability-waiver");
  }
  ...
  redirect(
    `/admin/settings?waiverSaved=1&version=${encodeURIComponent(version)}${textChanged && !manualVersion ? "&bumped=1" : ""}#liability-waiver`
  );
}
...
async function restoreRevision(formData: FormData) {
  ...
  if (!revision) {
    redirect("/admin/settings?waiverError=revision_missing#liability-waiver");
  }
  ...
  revalidatePath("/admin/settings");
  revalidatePath("/admin");
  redirect(`/admin/settings?restored=${encodeURIComponent(revision.version)}#liability-waiver`);
}
...
export default async function WaiverSettings(props: PageProps) {
  ...
  return (
    <section id="liability-waiver" aria-label="Liability Waiver" className="scroll-mt-4">
      <PageSection title="Liability Waiver" tone="muted">
        <p className="text-sm text-stone-500">The document, who has signed it, and every version you have published.</p>
      </PageSection>
